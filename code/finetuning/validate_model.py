...
def main():
    ...
    for idx in range(epoch_count):
        epoch_dir = os.path.join(args.input_dir, "epoch_" + str(idx))
        accelerator.print(f"Resumed from checkpoint: {epoch_dir}")
        accelerator.load_state(epoch_dir)
        ...
        for step, batch in enumerate(val_dataloader):
            ...
            data = raw_datasets['validation'][step*args.per_device_eval_batch_size : (step+1)*args.per_device_eval_batch_size]
    ...
    for i in range(len(bleu_scores)):
        print("EPOCH " + str(i))
        print("BLEU", bleu_scores[i])
        print("SARI", sari_scores[i])
        print()
