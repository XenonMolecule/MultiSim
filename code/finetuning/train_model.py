...
def parse_args():
    ...
    parser.add_argument(
        "--eval_each_epoch",
        action="store_true",
        help="Run eval metrics for every epoch to demonstrate progress (but take time)"
    )
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained('google/mt5-base', use_fast=not args.use_slow_tokenizer)
    ...
    def preprocess_function(examples):
        ...
        targets = [ex['simplifications'][0] for ex in examples[simple_column]]
        ...
        if (args.eval_each_epoch):
            for step, batch in enumerate(eval_dataloader):
                with torch.no_grad():
                    ...
                    inputs, generated_tokens, labels = accelerator.gather_for_metrics((inputs, generated_tokens, labels))
