...
def parse_args():
    ...
    parser.add_argument("--output_path", type=str, default=None, help="Where to write the model outputs to.")
    parser.add_argument("--metric_output_path", type=str, default=None, help="Where to write the metric outputs to.")
    parser.add_argument(
        "--model_type",
        type=str,
        default=None,
    ...
    parser.add_argument("--NC", type=float, default=1.0, help="Character compression token val")
    parser.add_argument("--LS", type=float, default=1.0, help="Levenshtein similarity token val")
    parser.add_argument("--DR", type=float, default=1.0, help="Depth ratio token val")
    parser.add_argument("--WR", type=float, default=1.0, help="Word rank token val")
...
def main():
    ...
    raw_datasets = load_dataset("./MultilingualSimplification.py", name=args.dataset_name, split="test")
    ...
    def preprocess_function(examples):
        ...
        prefix = "<NC_" + str(round(args.NC,2)) +"> <LS_" + str(round(args.LS,2)) +"> <DR_" + str(round(args.DR,2)) + "> <WR_ " + str(round(args.WR,2)) + "> "
    ...
    logger.info(f"  Num examples (val) = {len(eval_dataset)}")
    logger.info(f"  Instantaneous batch size per device = {args.per_device_eval_batch_size}")
    logger.info(f"  Batches to evaluate = {len(eval_dataloader)}")
    ...
    if args.metric_output_path:
        with open(args.metric_output_path, 'a') as f:
            ...
            f.write("SARI " + str(sari) + "\n")

    df = pd.DataFrame({"original": input_list, "mt5 output": out_list})
    if args.output_path:
        try:
            df.to_csv(args.output_path)
        except:
            print("OUTPUT PATH INVALID.  OUTPUTTING TO INPUT DIR")
            df.to_csv(os.path.join(args.input_dir, "output.csv"))
    else:
        print("NO OUTPUT PATH PROVIDED.  OUTPUTTING TO INPUT DIR")
        df.to_csv(os.path.join(args.input_dir, "output.csv"))
